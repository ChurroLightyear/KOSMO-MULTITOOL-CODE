...
//This selects the different menu modes to use. If you don't want them. slash them and remove the .ino and .h from the folder
...
void setup(void) {
  ...
  MIDI.setHandleControlChange(handleSustain);
...
///TEST
bool voiceflag[] = {false, false, false, false};
byte notecvtest[4];
byte lastPlayed;
byte polyphony = 3;
int robin = -1;
...
void handleSustain (byte channel, byte number, byte value) {
  if (channel != MIDI_CHANNEL && number != 64) return;
  if (value >= 64) {
    sustain = true;
  }
  if (value < 64) {
...
void handleNoteOn(byte channel, byte note, byte velocity) {
  ...
  if (MIDI_NOTES_ACTIVE.size() > polyphony) {
    return;
  }
  ...
  if (robinMode == 1) {
    ...
    while (voiceflag[robin] == true) {
      robin++;
      if (robin >= polyphony) {
        robin = 0;
      }
    }
    if (voiceflag[robin] == false) {
      notecvtest[robin] = note;
      voiceflag[robin] = true;
      lastPlayed = robin;
      return;
    }
  }
  ...
  if (robinMode == 3) {
    ...
    int randVoice = random(0, polyphony);

    while (voiceflag[randVoice] == true) {
      randVoice = random(0, polyphony);
    }


    if (voiceflag[randVoice] == false) {
      notecvtest[randVoice] = note;
      voiceflag[randVoice] = true;
      lastPlayed = randVoice;
      return;
    }
  }
}
...
void handleNoteOff(byte channel, byte note, byte velocity) {
  ...
  ///note off flag
  for (int x = 0; x < polyphony; x++) {
    if (notecvtest[x] == note) {
      if (voiceflag[x] == true) {
        voiceflag[x] = false;
        return;
      }
...
float getCVFromNote(float note) {
